...
import { type SearchBar_FormInput } from "@/lib/zod_schemas/search-bar.draft";

import { draft_getHotelsBySearchBarForm } from "@/lib/generated/prisma/sql";
...
export async function fetchSearchResult(
  ...
  console.log("fetchSearchResult called with filter:", filterFormValues);
  const {
  ...
  const result = await prisma.$queryRawTyped(draft_getHotelsBySearchBarForm(
    locationId,
    checkInDate,
    checkOutDate,
    numAdults,
  ...
  console.log("fetchSearchResult result:", { items, totalCount, lastItem });
  return {
    items,
    totalCount,
    nextCursor: lastItem ?
      {
        lastPrice: sort === 'price_desc'
          ? (lastItem.maxPrice?.toNumber() ?? FILTER_MAX_PRICE)   // FIXME: there is something wrong on first load. It can't return an empty array.
