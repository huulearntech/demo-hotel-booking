...
import { hotelowner_updateHotelName } from "@/lib/actions/hotel-manager/change-hotel-name";

export default function ChangeHotelNameDialog({ originalName }: { originalName: string }) {
  ...
  const onSubmit = (formValues: z.infer<typeof userUpdateNameSchema>) => {
    ...
    startTransition(async () => {
      const res = await hotelowner_updateHotelName(formValues);
      if (res.ok) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
          ...
          <div
           ...
           >
            <div className="flex flex-col">
              <span className="text-sm font-medium">Tên cơ sở lưu trú</span>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Thay đổi tên cơ sở lưu trú</DialogTitle>
          <DialogDescription>Cập nhật tên cơ sở lưu trú của bạn ở dưới đây.</DialogDescription>
        </DialogHeader>
