...
type FacilityCreateFormValues = {
  name: string;
  type: FacilityType;
};

export default function CreateFacilityDialog({ onCreate, roomTypeId }: {
  onCreate: (facility: { id: string; name: string; type: FacilityType }) => void;
  roomTypeId?: string;
}) {
  ...
  const onSubmit = async (values: FacilityCreateFormValues) => {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: [CACHE_TAGS.hotelowner_custom_facilities, CACHE_TAGS.hotelowner_custom_facilities_of_room_type]})
      onCreate({ id: result.data.id, name: result.data.name, type: result.data.type });
